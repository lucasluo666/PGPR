...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, dw_state_dim, act_dim, gamma=0.99, hidden_sizes=[512, 256]):
        super(ActorCritic, self).__init__()
        self.state_dim = state_dim
        self.dw_state_dim = dw_state_dim
        self.act_dim = act_dim
        self.gamma = gamma

        self.l1 = nn.Linear(state_dim, hidden_sizes[0])
        self.l2 = nn.Linear(hidden_sizes[0], hidden_sizes[1])
        self.dw_l1 = nn.Linear(dw_state_dim, hidden_sizes[0])
        self.dw_l2 = nn.Linear(hidden_sizes[0], hidden_sizes[1])
    ...
    def forward(self, inputs):
        state, dw_state, act_mask = inputs  # state: [bs, state_dim], act_mask: [bs, act_dim]
        x = self.l1(state)
        x = F.dropout(F.elu(x), p=0.5)
        out = self.l2(x)
        x = F.dropout(F.elu(out), p=0.5)

        dw_x = self.dw_l1(dw_state)
        dw_x = F.dropout(F.elu(dw_x), p=0.5)
        dw_out = self.dw_l2(dw_x)
        dw_x = F.dropout(F.elu(dw_out), p=0.5)
    ...
    def select_action(self, batch_state, dw_batch_state, batch_act_mask, device):
        state = torch.FloatTensor(batch_state).to(device)  # Tensor [bs, state_dim]
        dw_state = torch.FloatTensor(dw_batch_state).to(device)  # Tensor [bs, state_dim]
        act_mask = torch.ByteTensor(batch_act_mask).to(device)  # Tensor of [bs, act_dim]

        probs, value = self((state, dw_state, act_mask))  # act_probs: [bs, act_dim], state_value: [bs, 1]
        m = Categorical(probs)
...
def train(args):
    ...
    model = ActorCritic(env.state_dim, env.dw_state_dim, env.act_dim, gamma=args.gamma, hidden_sizes=args.hidden).to(args.device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            ...
            batch_state, dw_batch_state = env.reset(batch_uids)  # numpy array of [bs, state_dim]
            done = False
            while not done:
                batch_act_mask = env.batch_action_mask(dropout=args.act_dropout)  # numpy array of size [bs, act_dim]
                batch_act_idx = model.select_action(batch_state, dw_batch_state, batch_act_mask, args.device)  # int
                batch_state, dw_batch_state, batch_reward, done = env.batch_step(batch_act_idx)
