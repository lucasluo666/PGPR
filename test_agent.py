...
def batch_beam_search(env, model, uids, device, topk=[25, 5, 1]):
    ...
    state_pool, dw_state_pool = env.reset(uids)  # numpy of [bs, dim]
    path_pool = env._batch_path  # list of list, size=bs
    probs_pool = [[] for _ in uids]
    model.eval()
    for hop in range(3):
        state_tensor = torch.FloatTensor(state_pool).to(device)
        dw_state_tensor = torch.FloatTensor(dw_state_pool).to(device)
        ...
        probs, _ = model((state_tensor, dw_state_tensor, actmask_tensor))  # Tensor of [bs, act_dim]
        ...
        if hop < 2:
            state_pool,  dw_state_pool= env._batch_get_state(path_pool)
...
def evaluate_paths(path_file, train_labels, test_labels):
    ...
    dw_embeds = load_embed_dw(args.dataset)
    ...
    dw_user_embeds = dw_embeds[USER]
    dw_purchase_embeds = dw_embeds[PURCHASE][0]
    dw_product_embeds = dw_embeds[PRODUCT]
    dw_scores = np.dot(dw_user_embeds + dw_purchase_embeds, dw_product_embeds.T)
    final_score = [x+y for x, y in zip(scores, dw_scores)]
    ...
    for path, probs in zip(results['paths'], results['probs']):
        ...
        if pid not in pred_paths[uid]:
            pred_paths[uid][pid] = []
        path_score = final_score[uid][pid]
    ...
    for uid in best_pred_paths:
        ...
        if args.add_products and len(top10_pids) < 10:
            train_pids = set(train_labels[uid])
            cand_pids = np.argsort(final_score[uid])
