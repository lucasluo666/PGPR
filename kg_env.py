...
class KGState(object):
    def __init__(self, embed_size, dw_embed_size, history_len=1):
        self.embed_size = embed_size
        self.dw_embed_size = dw_embed_size
        self.history_len = history_len  # mode: one of {full, current}
        if history_len == 0:
            self.dim = 2 * embed_size
            self.dw_dim = 2 * dw_embed_size
        elif history_len == 1:
            self.dim = 4 * embed_size
            self.dw_dim = 4 * dw_embed_size
        elif history_len == 2:
            self.dim = 6 * embed_size
            self.dw_dim = 6 * dw_embed_size
        else:
    ...
    def __call__(self, user_embed, node_embed, last_node_embed, last_relation_embed, older_node_embed,
                 older_relation_embed, dw_user_embed, dw_node_embed, dw_last_node_embed, dw_last_relation_embed, dw_older_node_embed,
                 dw_older_relation_embed):
        if self.history_len == 0:
            return np.concatenate([user_embed, node_embed],axis=-1), np.concatenate([dw_user_embed, dw_node_embed],axis=-1)
        elif self.history_len == 1:
            return np.concatenate([user_embed, node_embed, last_node_embed, last_relation_embed], axis=-1),np.concatenate([dw_user_embed, dw_node_embed, dw_last_node_embed, dw_last_relation_embed], axis=-1)
        elif self.history_len == 2:
            return np.concatenate([user_embed, node_embed, last_node_embed, last_relation_embed, older_node_embed,
                                   older_relation_embed], axis=-1),np.concatenate([dw_user_embed, dw_node_embed, dw_last_node_embed, dw_last_relation_embed, dw_older_node_embed,
                                   dw_older_relation_embed], axis=-1)
        else:
...
class BatchKGEnvironment(object):
    def __init__(self, dataset_str, max_acts, max_path_len=3, state_history=1):
        ...
        self.dw_embeds = load_embed_dw(dataset_str)
        self.embed_size = self.embeds[USER].shape[1]
        self.dw_embed_size = self.dw_embeds[USER].shape[1]
        self.embeds[SELF_LOOP] = (np.zeros(self.embed_size), 0.0)
        self.dw_embeds[SELF_LOOP] = (np.zeros(self.dw_embed_size), 0.0)
        self.state_gen = KGState(self.embed_size, self.dw_embed_size, history_len=state_history)
        self.state_dim = self.state_gen.dim
        self.dw_state_dim = self.state_gen.dw_dim
        # self.dw_state_dim = self.state_gen.dw_dim

        # Compute user-product scores for scaling.
        u_p_scores = np.dot(self.embeds[USER]+self.embeds[PURCHASE][0],self.embeds[PRODUCT].T)
        dw_u_p_scores = np.dot(self.dw_embeds[USER]+self.dw_embeds[PURCHASE][0], self.dw_embeds[PRODUCT].T)

        self.u_p_scales = np.max(u_p_scores, axis=1)
        self.dw_u_p_scales = np.max(dw_u_p_scores, axis=1)
    ...
    def _get_actions(self, path, done):
        ...
        dw_scores= []
        user_embed = self.dw_embeds[USER][path[0][-1]]
        for r, next_node_id in candidate_acts:
            next_node_type = KG_RELATION[curr_node_type][r]
            if next_node_type == USER:
                src_embed = user_embed
            elif next_node_type == PRODUCT:
                src_embed = user_embed + self.dw_embeds[PURCHASE][0]
            elif next_node_type == WORD:
                src_embed = user_embed +self.dw_embeds[MENTION][0]
            else:  # BRAND, CATEGORY, RELATED_PRODUCT
                src_embed = user_embed + self.dw_embeds[PURCHASE][0] + self.dw_embeds[r][0]
            dw_score = np.matmul(src_embed, self.dw_embeds[next_node_type][next_node_id])

            dw_scores.append(dw_score)

        result = [x + y for x, y in zip(scores, dw_scores)]

        candidate_idxs = np.argsort(result)[-self.max_acts:]  # choose actions with larger scores
    ...
    def _get_state(self, path):
        ...
        user_embed = self.embeds[USER][path[0][-1]]
        dw_user_embed =  self.dw_embeds[USER][path[0][-1]]
        zero_embed = np.zeros(self.embed_size)
        dw_zero_embed = np.zeros(self.dw_embed_size)
        if len(path) == 1:  # initial state
            state, dw_state = self.state_gen(user_embed, user_embed, zero_embed, zero_embed, zero_embed, zero_embed, dw_user_embed, dw_user_embed, dw_zero_embed, dw_zero_embed, dw_zero_embed, dw_zero_embed)
            return (state, dw_state)
        ...
        dw_curr_node_embed = self.dw_embeds[curr_node_type][curr_node_id]
        dw_last_node_embed = self.dw_embeds[last_node_type][last_node_id]
        dw_last_relation_embed = self.dw_embeds[last_relation][0]

        if len(path) == 2:
            state, dw_state = self.state_gen(user_embed, curr_node_embed, last_node_embed, last_relation_embed, zero_embed,
                                   zero_embed, dw_user_embed, dw_curr_node_embed, dw_last_node_embed, dw_last_relation_embed,dw_zero_embed, dw_zero_embed)
            return (state, dw_state)
        ...
        dw_older_node_embed = self.dw_embeds[older_node_type][older_node_id]
        dw_older_relation_embed = self.dw_embeds[older_relation][0]

        state, dw_state = self.state_gen(user_embed, curr_node_embed, last_node_embed, last_relation_embed, older_node_embed,older_relation_embed,
                                         dw_user_embed, dw_curr_node_embed, dw_last_node_embed, dw_last_relation_embed, dw_older_node_embed, dw_older_relation_embed)
        return (state, dw_state)

    def _batch_get_state(self, batch_path):
        batch_state, dw_batch_state = [], []
        for path in batch_path:
            state, dw_state=self._get_state(path)
            batch_state.append(state)
            dw_batch_state.append(dw_state)
        return np.vstack(batch_state) , np.vstack(dw_batch_state) # [bs, dim]
    ...
    def _get_reward(self, path):
        ...
        if curr_node_type == PRODUCT:
            ...
            u_vec = self.dw_embeds[USER][uid] + self.dw_embeds[PURCHASE][0]
            p_vec = self.dw_embeds[PRODUCT][curr_node_id]
            dw_score = np.dot(u_vec, p_vec) / self.dw_u_p_scales[uid]
            target_score = max(score+dw_score, 0.0)
    ...
    def reset(self, uids=None):
        ...
        self._done = False
        self._batch_curr_state,self._dw_batch_curr_state = self._batch_get_state(self._batch_path)
        ...
        return self._batch_curr_state, self._dw_batch_curr_state
    ...
    def batch_step(self, batch_act_idx):
        ...
        return self._batch_curr_state,self._dw_batch_curr_state, self._batch_curr_reward, self._done
